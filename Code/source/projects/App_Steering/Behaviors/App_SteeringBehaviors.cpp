	...
	//UI
	{
		...
		for (UINT i = 0; i < m_AgentVec.size(); ++i)
		{
			...
			if (ImGui::CollapsingHeader(headerName))
			{
				...
				if (ImGui::Combo("", &a.SelectedBehavior, "Seek\0Wander\0Flee\0Arrive\0Face\0Evade\0Pursuit", 6)) //Add your steering behaviours right here to the dropdownlist
				{
...
void App_SteeringBehaviors::SetAgentBehavior(ImGui_Agent& a)
{
	...
	bool autoOrient = true; //IF rotational behavior you might want to false this one
	...
	case BehaviorTypes::Pursuit:
		...
		break;
	//... //change your code here for behaviots aswelll as in adding your behaviour case
	}

	if (useMouseAsTarget)
		a.pBehavior->SetTarget(m_Target);
	else
		a.pBehavior->SetTarget(m_AgentVec[a.SelectedTarget].pAgent->GetPosition());

	a.pAgent->SetAutoOrient(autoOrient);
	a.pAgent->SetSteeringBehavior(a.pBehavior);
}

void App_SteeringBehaviors::UpdateBehaviorTarget()
{
	for (auto& a : m_AgentVec)
	{
		bool useMouseAsTarget = a.SelectedTarget < 0;
