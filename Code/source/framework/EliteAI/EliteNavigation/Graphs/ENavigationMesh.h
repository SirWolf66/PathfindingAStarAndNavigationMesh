...
namespace Elite
{
	...
	public:
		...
		void ExpandLinesReviewedContainer(unsigned int size) { m_ReviewedLines.resize(size); }
...
#pragma region BasicFunctions
		...
		const Triangle* GetClosestTriangle(const Vector2& position) const
		{
			...
			{
				auto const currentDistanceSqrt = (pT->GetCenter() - position).SqrtMagnitude();
		...
		const Triangle* GetCurrentTriangleFromPosition(const Vector2& position, bool onLineAllowed = false) const
		{
			for (auto pT : m_pNavMeshPolygon->GetTriangles())
			{
				auto const inTriangle = PointInTriangle(position, pT->p1, pT->p2, pT->p3, onLineAllowed);
				if (inTriangle)
					return pT;
			}
			return nullptr;
